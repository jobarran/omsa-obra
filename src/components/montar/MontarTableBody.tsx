...
export const MontarTableBody = ({ duplicatedCodes }: Props) => {
    ...
    const isLoadingMaterial = useMaterialStore(state => state.isLoadingMaterial)
            ...
            <table className="border-collapse text-left text-sm text-gray-500 w-full">
                <thead className="text-md text-uppercase table-header-group">
                    <tr>
                        ...
                        <th className="px-4 py-4 font-semibold text-gray-900 sm:w-1/6 md:w-1/4 lg:w-1/3 text-center">Posición</th>
                        ...
                        <tbody className="divide-y divide-gray-100 border-t border-gray-100">
                            {storeMaterial?.map((item) => (
                                <tr key={item.code+item.name[0]} className={duplicatedCodes?.includes(item.code) ? "bg-red-100" : "hover:bg-gray-50"}>
                                    ...
                                    <td className="px-2 py-4 text-center">{item.possition}</td>
